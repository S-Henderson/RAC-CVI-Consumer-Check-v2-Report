...
# For tracking - total hits before removing duplicates
...
# For report tracking
worked_claims_sum <- df_exceptions %>%
  summarize(
    `Sum of TAG Transactions` = sum(`Claim Amount`)
  )

# Remove worked claims sum -----

# To match App Support formatting for file
...
print("Saving CVI Exceptions File")
...
print("Saving Built Report File")
...
print(paste0(total_hits_count, " - Total Hits"))         # Counts total hits of report (from earlier calculation)

print(paste0(nrow(df)," - Total Assessed"))              # Counts total assessed of report

print(paste0(nrow(df_exceptions)," - Total Actioned"))   # Counts total actioned (TAG transactions)

print(paste0(worked_claims_sum[[1]], " - Worked Value")) # Sum of worked value (calculated from exceptions)
...
# Open App Support Request Link -----

print("Opening Request Portal Link")

# To send exceptions file to App Support -> opens in default browser
browseURL("https://360insights.atlassian.net/servicedesk/customer/portal/28/group/105/create/520")
